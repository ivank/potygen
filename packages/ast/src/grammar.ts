...
const Select = Y((SelectExpression) => {
   ...
   * ----------------------------------------------------------------------------------------
   */
  const CombinationType = astUpperLeaf<Tag.CombinationTypeTag>('CombinationType', /^(UNION|INTERSECT|EXCEPT)/i);
...
const CTE = astNode<Tag.CTETag>('CTE', All(Identifier, /^AS/, Brackets(Query)));
