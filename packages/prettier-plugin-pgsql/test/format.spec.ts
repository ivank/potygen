...
describe('Format', () => {
  ...
  it('should parse multiple inner joins correctly', () => {
    const nerQuery = 'SELECT 1 FROM table1 INNER JOIN foo ON table1.foo_id = foo.id INNER JOIN bar ON table1.bar_id = bar.id'
    const formatted = format(nerQuery, { parser: 'sql', plugins: [prettierPluginPgSql] });
    const wanted = `
SELECT 1
FROM
  table1
  INNER JOIN foo
    ON table1.foo_id = foo.id IN NER
  JOIN bar
    ON table1.bar_id = bar.id`
    expect(formatted).toEqual(wanted)
  })
});
