  ...
  JOIN contacts ON contacts.id = customers.primary_contact_id
  JOIN addresses ON addresses.id = contacts.address_id
  JOIN contracts ON contracts.account_id = accounts.id
  JOIN installations ON contracts.installation_id = installations.id
  JOIN addresses AS installation_addresses ON installations.address_id = installation_addresses.id
  ...
  WHERE
    ...
    ($q = '' OR (
      ...
      OR to_tsvector(
        'english',
        CONCAT_WS(' ', addresses.address_line_1, addresses.address_line_2, addresses.city)
      ) @@ plainto_tsquery('english', $q)

      OR to_tsvector(
        'english',
        CONCAT_WS(' ', installation_addresses.address_line_1, installation_addresses.address_line_2, installation_addresses.city, installation_addresses.postcode)
      ) @@ plainto_tsquery('english', $q)
    ...
    AND (cardinality($ids::int[]) = 0 OR (accounts.id = ANY($ids::int[])))
  ...
  GROUP BY (
    ...
    contacts.first_name,
    contacts.last_name,
    contacts.email,installation_addresses.address_line_1,
    installation_addresses.address_line_2,
    installation_addresses.city,
    installation_addresses.postcode
  )
