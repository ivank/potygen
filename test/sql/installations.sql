...
SELECT
  ...
  (
    ...
    SELECT ivi."DateVerified" FROM fit.InstallationVerificationInfo ivi WHERE ivi."InstallationId" = i."InstallationId" ORDER BY ivi."DateVerified" ASC LIMIT 1
  ),
  (
    ...
    FROM fit.InstallationVerificationInfo ivi
    WHERE ivi."InstallationId" = i."InstallationId"
    GROUP BY "InstallationId"
    HAVING COUNT("InstallationId") > 1
  ),
  i."ConfirmationDate",
  ...
  (
    ...
    JOIN fit.Meter m ON m."MeterId" = im."MeterId"
  ...
  (
    SELECT s."Company"
    FROM fit.Switch s
    WHERE s."InstallationId" = i."InstallationId" AND s."IsSwitchIn" = TRUE
  ...
  (
    SELECT s."Company"
    FROM fit.Switch s
    WHERE s."InstallationId" = i."InstallationId" AND s."IsSwitchIn" = FALSE
    ORDER BY s."DateInserted" DESC
    LIMIT 1
  ),
  (
    ...
    FROM fit.Switch s
    WHERE s."InstallationId" = i."InstallationId"
    ORDER BY s."DateInserted" DESC
    LIMIT 1
  ),
  ...
  i."DateInserted",
  i."DateUpdated"
FROM fit.Installation i
JOIN fit.InstallationType it ON it."InstallationTypeId" = i."InstallationTypeId"
JOIN fit.GeneratorTechnologyType gtt ON gtt."GeneratorTechnologyTypeId" = i."GeneratorTechnologyTypeId"
JOIN fit.PropertyType pt ON pt."PropertyTypeId" = i."PropertyTypeId"
JOIN fit.GenerationAccountInstallation gai ON gai."InstallationId" = i."InstallationId"
JOIN fit.GenerationAccount ga ON ga."GenerationAccountId" = gai."GenerationAccountId"
...
LEFT JOIN fit.Customer c ON c."CustomerId" = ca."CustomerId"
